...
public class FollowJoystick : MonoBehaviour {
	...
	public void MoveByForce(Vector3 normalizedVector, bool relative = false){
		if (!relative)
			rigidbody.AddForce (normalizedVector * moveForce*Time.fixedDeltaTime , ForceMode.VelocityChange);
		else
			rigidbody.AddRelativeForce(normalizedVector * moveForce*Time.fixedDeltaTime , ForceMode.VelocityChange);
	}
